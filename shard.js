var shard = parseInt(process.env["SHARD_ID"]) + 1;
var shards = parseInt(process.env["SHARD_COUNT"]);
...
for (var i in files) {
  if (files[i].endsWith(".js")) {
    var temp = require(path.join(__dirname, "./commands/") + files[i]);
    for (var i in temp) {
      commands.set(i, temp[i]);
    }
  }
...
Client.on("guildCreate", g => {
  antiBotCollection(g, true);
});
Client.on("guildMemberAdd", m => {
  antiBotCollection(m);
  db.fetchGuildData(m.guild.id, {option: "welcome"}).then(r => {
    r.toArray().then(array => {
      if (array[0]) {
        if (array[0].state === "true") {
...
Client.on("guildMemberRemove", antiBotCollection);
    ...
    if (m.content.startsWith(config.Constants.prefix)) {
      ...
      if (m.guild) {
        if (m.guild.ownerID === m.author.id) {
          ...
          db.fetchGuildData(m.guild.id, {user: m.author.id}).then(r => {
            r.toArray().then(a => {
              if (a[0]) {
                level = a[0].level;
                if (config.permissions.master.indexOf(m.author.id) > -1) {
      ...
      function go () { //eslint-disable-line
        ...
        if (commands.has(cmd)) {
          try {
            if (commands.get(cmd).guild && !m.guild) return m.channel.sendMessage(":x: **This command is intended for in-server use!**").catch(e => {});
            if (commands.get(cmd).level > level) {
            ...
            } else commands.get(cmd).fn(Client, m, suffix, db);
